 ...
 return (
  ...
  >
   <ListboxButton className="relative w-full cursor-pointer rounded-lg border border-black/10 text-left text-neutral-700 duration-200 hover:border-black/30 hover:text-neutral-800 motion-reduce:transition-none sm:text-sm dark:border-neutral-800 dark:text-neutral-200/75 dark:hover:border-neutral-800 dark:hover:text-neutral-200">
    <span className="flex truncate py-2 pr-10 pl-2">{text}</span>
   ...
   <ListboxOptions className="absolute z-50 mt-1 max-h-60 w-[var(--button-width)] origin-top overflow-auto rounded-md border border-black/10 bg-white/90 py-1 text-base shadow-lg backdrop-blur duration-100 ease-in data-[closed]:scale-95 data-[closed]:opacity-0 motion-reduce:transition-none sm:text-sm dark:border-neutral-800 dark:bg-[#161617]/90">
    {options.map((option) => (
     <ListboxOption
      key={option.value}
      value={option.value}
      disabled={option.disabled}
      className="relative flex truncate py-2 text-left text-neutral-800 duration-200 select-none data-[disabled]:opacity-50 data-[focus]:bg-black/10 motion-reduce:transition-none dark:text-white data-[focus]:dark:bg-white/10"
     >
      {option.text}
     </ListboxOption>
    ))}
